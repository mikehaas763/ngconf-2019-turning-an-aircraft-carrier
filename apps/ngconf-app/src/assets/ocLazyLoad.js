...
(function() {
  ...
  ocLazyLoad.provider('$ocLazyLoad', [
    ...
    function(
        ...
        function(
          ...
          providers.getInstanceInjector = function() {
            return instanceInjector
              ? instanceInjector
              : (instanceInjector =
                  $rootElement.data('$injector') || angular.injector());
          };
              ...
              angular.forEach(paths, function(url) {
                ...
                $http
                  ...
                  .error(function(err) {
                    deferred.reject(
                      new Error(
                        'Unable to load template file "' + url + '": ' + err
                      )
                    );
                  });
              });
          ...
          var filesLoader = function filesLoader(config, params) {
            ...
            var pushFile = function(path) {
              ...
              if (angular.isUndefined(cachePromise) || params.cache === false) {
                ...
                if (!file_type) {
                  if (
                    (m = /[.](css|less|html|htm|js)?(\?.*)?$/.exec(path)) !==
                    null
                  ) {
                    // Detect file type via file extension
                    file_type = m[1];
                  } else if (
                    !jsLoader.hasOwnProperty('ocLazyLoadLoader') &&
                    jsLoader.hasOwnProperty('load')
                  ) {
                    // requirejs
                    file_type = 'js';
                  } else {
                ...
                if (
                  (file_type === 'css' || file_type === 'less') &&
                  cssFiles.indexOf(path) === -1
                ) {
                  cssFiles.push(path);
                } else if (
              ...
              cssLoader(
                ...
                function(err) {
                  if (
                    ...
                    cssLoader.hasOwnProperty('ocLazyLoadLoader')
                  ) {
                    $log.error(err);
            ...
            if (templatesFiles.length > 0) {
              ...
              templatesLoader(
                ...
                function(err) {
                  if (
                    angular.isDefined(err) &&
                    templatesLoader.hasOwnProperty('ocLazyLoadLoader')
                  ) {
                    $log.error(err);
          ...
          return {
            ...
            load: function(module, params) {
              ...
              if (typeof module === 'string') {
                ...
                if (
                  angular.isDefined(module.path) &&
                  angular.isDefined(module.type)
                ) {
                ...
                angular.forEach(requires, function(requireEntry) {
                  ...
                  if (moduleExists(requireEntry.name)) {
                    if (typeof module !== 'string') {
                      ...
                      promisesList.push(
                        filesLoader(requireEntry.files, localParams).then(
                          function() {
                    ...
                    if (
                      requireEntry.hasOwnProperty('name') &&
                      requireEntry['name']
                    ) {
                  ...
                  if (
                    requireEntry.hasOwnProperty('files') &&
                    requireEntry.files.length !== 0
                  ) {
                    if (requireEntry.files) {
                      promisesList.push(
                        filesLoader(requireEntry, localParams).then(function() {
                          return loadDependencies(requireEntry);
                        })
                      );
                    }
              ...
              filesLoader(config, localParams).then(
                function success() {
                  if (modulesToLoad.length === 0) {
                    ...
                    var loadNext = function loadNext(moduleName) {
                      ...
                      loadDependencies(moduleName).then(
                        function success() {
                          try {
                            justLoaded = [];
      ...
      this.config = function(config) {
        if (
          angular.isDefined(config.jsLoader) ||
          angular.isDefined(config.asyncLoader)
        ) {
            ...
            $scope.$watch(
              ...
              function(moduleName) {
                if (angular.isDefined(moduleName)) {
                  $ocLazyLoad
                    .load(moduleName)
                    .then(function success(moduleConfig) {
  ...
  function register(providers, registerModules, params) {
    if (registerModules) {
      ...
      for (k = registerModules.length - 1; k >= 0; k--) {
        ...
        if (
          angular.isDefined(runBlocks[moduleName]) &&
          (newModule || params.rerun)
        ) {
        ...
        broadcast(
          newModule ? 'ocLazyLoad.moduleLoaded' : 'ocLazyLoad.moduleReloaded',
          moduleName
        );
        registerModules.pop();
  ...
  function registerInvokeList(args, moduleName) {
    ...
    if (angular.isString(invokeList)) {
      ...
      if (
        regInvokes[moduleName][type][invokeList].indexOf(
          signature(args[2][1])
        ) === -1
      ) {
      ...
      angular.forEach(invokeList, function(invoke) {
        if (angular.isString(invoke)) {
          ...
          if (
            regInvokes[moduleName][type][invoke].indexOf(
              signature(invokeList[1])
            ) === -1
          ) {
    ...
    if (modulesToLoad.length === 0) {
      ...
      angular.forEach(names, function(name) {
        ...
        if (element[0].querySelectorAll) {
          ...
          angular.forEach(
            element[0].querySelectorAll('.' + name + '\\:'),
            append
          );
          angular.forEach(
            element[0].querySelectorAll('[' + name + ']'),
